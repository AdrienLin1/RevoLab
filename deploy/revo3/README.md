...
- `revo3_deploy.tactile_exporter`: exports the HORA tactile DAgger student and normalizer.
- `revo3_deploy.tactile_policy_runner`: builds physical-taxel histories and runs tactile ONNX.
- `revo3_deploy.task_registry`: aligns task names with tactile fingers, modules, dimensions, and action masks.
...
- `scripts/inspect_hand.py`: prints all joints and every SDK touch channel per frame.
- `scripts/run_tactile_policy.py`: runs the physical-taxel tactile student.

For the current tactile student contract and hardware procedure, read
[`TACTILE_DEPLOYMENT_ZH.md`](TACTILE_DEPLOYMENT_ZH.md). The exact Revo3-facing
API surface inspected from `bc-stark-sdk==1.4.5` is recorded in
[`BC_STARK_SDK_1_4_5_ZH.md`](BC_STARK_SDK_1_4_5_ZH.md).
