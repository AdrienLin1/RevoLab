...
class Revo3HandHoraEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Tactile ball/cylinder tasks keep rewarding every increase in positive speed.
        rotate_reward = compute_rotate_reward(
            (object_angvel * self.rot_axis).sum(-1),
            float(self.cfg.angvel_clip_min),
            float(self.cfg.angvel_clip_max),
            bool(self.cfg.unbounded_positive_rotate_reward),
        )
        ...
        self.extras['metrics/angular_velocity_per_env'] = (
            object_angvel * self.rot_axis
        ).sum(dim=-1).detach()
    ...
    def compute_observations(self):
        ...
        if bool(getattr(self.cfg, "enable_contact_noise", False)) and not self.cfg.binary_contact:
            smooth_forces_w = (
            ...
            smooth_forces_w = _apply_contact_force_vector_noise(
                smooth_forces_w,
                float(getattr(self.cfg, "contact_force_noise_frac", 0.0)),
            )
            smooth_contact_forces = torch.linalg.vector_norm(smooth_forces_w, dim=-1)
        else:
        ...
        public_command = self._get_public_command_observation()
        if public_command is not None:
            if public_command.ndim != 2 or public_command.shape[0] != self.num_envs:
                raise RuntimeError(
                    "Public command observation must have shape (num_envs, command_dim), "
                    f"got {tuple(public_command.shape)}"
                )
            cur_obs_buf = torch.cat([cur_obs_buf, public_command.unsqueeze(1)], dim=-1)
        expected_frame_dim = self.cfg.observation_space // 3
        if cur_obs_buf.shape[-1] != expected_frame_dim:
            raise RuntimeError(
                f"Observation frame width {cur_obs_buf.shape[-1]} != configured "
                f"{expected_frame_dim}"
            )
        ...
        if public_command is not None:
            command_start = ndof * 2 + 5
            command_end = command_start + public_command.shape[-1]
            self.obs_buf_lag_history[at_reset_env_ids, :, command_start:command_end] = (
                public_command[at_reset_env_ids].unsqueeze(1)
            )
        ...
        if not self.cfg.enable_contact_in_obs:
            obs_single = self.cfg.observation_space // 3
            for f in range(3):
    ...
    def _get_public_command_observation(self) -> torch.Tensor | None:
        """Return optional public command channels appended to every observation frame."""
        return None
...
def _apply_contact_force_vector_noise(
    forces: torch.Tensor,
    noise_frac: float,
) -> torch.Tensor:
    """Add relative-magnitude random-direction noise to contact forces.

    Args:
        forces: Force vectors with final dimension three.
        noise_frac: Noise magnitude as a fraction of each force norm.

    Returns:
        Noisy force vectors with zero-force entries preserved.
    """
    if noise_frac <= 0.0:
        return forces
    magnitude = torch.linalg.vector_norm(forces, dim=-1, keepdim=True)
    direction = torch.randn_like(forces)
    direction = direction / torch.linalg.vector_norm(
        direction,
        dim=-1,
        keepdim=True,
    ).clamp_min(1.0e-8)
    return forces + noise_frac * magnitude * direction


def compute_rotate_reward(
    axis_angvel: torch.Tensor,
    clip_min: float,
    clip_max: float,
    unbounded_positive: bool,
) -> torch.Tensor:
    """Compute the axis-rotation reward before applying its configured scale.

    Args:
        axis_angvel: Object angular velocity projected onto the desired axis.
        clip_min: Lower reward bound for reverse rotation.
        clip_max: Upper reward bound used by legacy bounded tasks.
        unbounded_positive: Whether positive angular velocity remains unclipped.

    Returns:
        Per-environment rotation reward.
    """
    if unbounded_positive:
        return torch.clamp_min(axis_angvel, clip_min)
    return torch.clamp(axis_angvel, min=clip_min, max=clip_max)
