...
Per-environment tensors whose key ends with ``_per_env`` are preserved because PPO
uses them as rollout reward streams rather than scalar logging values.
"""
    ...
    def step(
        ...
        for k, v in getattr(self._env, "extras", {}).items():
            if isinstance(v, torch.Tensor):
                if isinstance(k, str) and k.endswith("_per_env"):
                    infos[k] = v
                else:
                    infos[k] = v.float().mean()
        infos["time_outs"] = truncated
        # Preserve per-environment termination causes for play-time robustness
        # metrics. PPO ignores this auxiliary tensor; ``time_outs`` remains the
        # canonical rollout flag used by the training code.
