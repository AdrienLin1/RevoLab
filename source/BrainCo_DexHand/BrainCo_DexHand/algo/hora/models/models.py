...
class ProprioAdaptTConv(nn.Module):
    def __init__(self, obs_per_step=47):
    ...
    def forward(self, x):
        x = self.channel_transform(x)  # (N, prop_hist_len, 32)
        x = x.permute((0, 2, 1))
...
class ActorCritic(nn.Module):
    def __init__(self, kwargs):
        ...
        if self.priv_info:
            ...
            if self.priv_info_stage2:
                self.adapt_tconv = ProprioAdaptTConv(obs_per_step=obs_per_step)
    ...
    def act(self, obs_dict):
        # training: sample from Normal(μ, σ)
