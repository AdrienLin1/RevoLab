...
本文档面向在 RevoLab 仓库中执行任务的 Cursor Agent。修改代码前请先阅读相关领域文档，
避免与已有架构冲突。
...
| 文档 | 路径 | 何时阅读 |
|---|---|---|
| **hora_screw 任务说明** | [`REVO_HORA_SCREW_TASKS.md`](REVO_HORA_SCREW_TASKS.md) | 启动/调试 nutbolt、screwdriver、触觉/阀门任务；Stage1 PPO 与 Stage2 蒸馏命令；环境设计与 DEXSCREW 对照；训练侧编码器类型/`pool_mode` 摘要 |
| **hora_screw 网络架构** | [`source/BrainCo_DexHand/BrainCo_DexHand/tasks/direct/hora_screw/Model.md`](source/BrainCo_DexHand/BrainCo_DexHand/tasks/direct/hora_screw/Model.md) | 修改 `ActorCritic`、`TactileStudentPolicy`、MLP/conv1d/GRU 触觉编码器；理解 obs/priv/extrin 维度与 Stage2 DAgger 差异 |
| **TacRes 方法方案** | [`FINAL_PROPOSAL_20260705_1547.md`](FINAL_PROPOSAL_20260705_1547.md) | Dexsuite Lift 上的 TacRes phase1/phase2 实现（见下文） |

### `Model.md` 覆盖范围（摘要）

- **Stage1 教师** `ActorCritic`：obs 141、priv 11/331、extrin 32、flat MLP 特权编码
- **Stage2 非触觉** `ProprioAdapt`：`adapt_tconv` 蒸馏 32d latent
- **Stage2 触觉** `TactileDAgger`：学生 proprio + 二进制触觉历史、conv1d/gru 学生编码器、动作 MSE 蒸馏
- 公开配置对照表与 checkpoint 兼容性说明见 `Model.md`
- 代码索引：`models.py`、`tactile_dagger.py`、`train.py`

任务级命令与 TacSL 环境细节以 `REVO_HORA_SCREW_TASKS.md` 为准；网络/维度/配置开关以 `Model.md` 为准。

---

## TacRes 实现任务（Dexsuite Lift）

以下内容为 Dexsuite-Revo3-Lift 上 TacRes 方法的 Agent 任务说明。
